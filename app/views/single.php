			...
			<nav class="px-6 py-4 bg-white shadow">
				<div class="container flex flex-col mx-auto md:flex-row md:items-center md:justify-between">
					<div class="flex items-center justify-between">
						<div>
							<a href="/"
							   class="text-xl font-bold text-gray-800 md:text-2xl">Brand</a>
						</div>
					</div>
								...
								<div class="flex items-center justify-between">
									<a href="?author=<?= $view[ 'data' ][ 'post' ] -> author_name ?>"
									   class="flex items-center justify-end"><img
												...
												alt="View posts by <?= ucwords ( $view[ 'data' ][ 'post' ] -> author_name ) ?>"
												class="hidden object-cover w-10 h-10 mr-4 rounded-full sm:block">
										<h1 class="font-bold text-gray-700 hover:underline"><?= ucwords ( $view[ 'data' ][ 'post' ] -> author_name ) ?></h1>
									</a>
									<a href="?category=<?= $view[ 'data' ][ 'post' ] -> categories -> category_slug ?>"
									   class="px-2 py-1 font-bold text-gray-100 bg-gray-600 rounded hover:bg-gray-500"><?= ucwords ( $view[ 'data' ][ 'post' ] -> category_name ) ?>
									</a>
								</div>
								<div class="my-4">
                                    <span class="font-light text-gray-600">
                                        <?= ( new Datetime( $view[ 'data' ][ 'post' ] -> published_at ) ) -> format ( 'M d, Y \a\t H:i' ) ?>
                                    </span>
								</div>
