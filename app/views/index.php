...
<html lang="en">
	...
	<body class="bg-gray-200">
		<div class="overflow-x-hidden bg-gray-100">
			<?php include PARTIALS_PATH . '_nav-not-connected.php' ?>
			<div class="px-6 py-8">
				<div class="container flex justify-between mx-auto">
					<div class="w-full lg:w-8/12">
						<div class="flex items-center justify-between">
							<h1 class="text-xl font-bold text-gray-700 md:text-2xl">Posts</h1>
							<?php include PARTIALS_PATH . '_sort-order-change-form.php' ?>
						</div>
						...
						<?php foreach ( $view[ 'data' ][ 'posts' ] as $post ): ?>
							<div class="mt-6">
								<div class="max-w-4xl px-10 py-6 mx-auto bg-white rounded-lg shadow-md">
									<div class="flex items-center justify-between">
										<span class="font-light text-gray-600"><?= ( new Datetime( $post -> published_at ) ) -> format ( 'M d, Y \a\t H:i' ) ?></span>
									...
									<div class="mt-2">
										<a href="?action=show&slug=<?= $post -> slug ?>"
									...
									<div class="flex items-center justify-between mt-4">
										<a href="?action=show&id=<?= $post -> id ?>"
										   class="text-blue-500 hover:underline">Read
																				 more<span class="sr-only"> about <?= $post -> title ?></span>
										</a>
										<div>
											<a href="?author=<?= $post -> author_name ?>"
											   class="flex items-center">
												<img src="<?= $post -> author_avatar ?>"
													 alt="View posts by <?= ucwords ( $post -> author_name ) ?>"
													 class="hidden object-cover w-10 h-10 mx-4 rounded-full sm:block">
												<span class="font-bold text-gray-700 hover:underline"><?= ucwords ( $post -> author_name ) ?></span>
											</a>
										</div>
									</div>
								</div>
